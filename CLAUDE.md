...
- **Il runner è un servizio `systemd --user`: serve il lingering** (`loginctl enable-linger`). Senza, dopo un riavvio della macchina resta spento finché qualcuno non entra via SSH — è già successo: server riavviato per un aggiornamento del kernel, tutto il resto tornato su da solo, runner giù per 67 minuti.
